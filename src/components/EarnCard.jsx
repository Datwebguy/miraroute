...
export default function SuccessOverlay({ open, onClose, fromSym, toSym, amountIn, amountOut, txHash, onGoEarn }) {
  if (!open) return null;

  const explorerUrl = txHash ? `${EXPLORER}/tx/${txHash}` : `${EXPLORER}`;
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center p-4 sm:p-6 anim-fadein">
      ...
      <div className="relative w-full max-w-[480px] anim-slideup">
        <div className="rounded-3xl bg-[#0F1E2E] card-stroke shadow-card p-6 relative overflow-hidden">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[11px] mono uppercase tracking-[0.18em] text-teal-400 mb-1">Swap confirmed on Arc</div>
              <div className="text-[18px] font-semibold tracking-tight">
                {fmt(amountIn)} {fromSym} → <span className="grad-text">{fmt(amountOut, 4)} {toSym}</span>
              </div>
              <div className="text-[12px] text-white/50 mt-2 flex items-center gap-2 mono">
                {txHash ? (
                  <>
            ...
            <button onClick={onClose} className="text-white/40 hover:text-white shrink-0 mt-1">
              <Icons.Close size={16}/>
            </button>
          </div>
                ...
                {[
                  ...
                  <div key={i} className="flex items-center gap-3 p-2.5 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] transition card-stroke cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[11px] mono text-white/45">{fmt(amountOut * 0.9, 2)} {toSym} available</div>
                    </div>
