...
export default function TokenSelector({ open, onClose, onPick, exclude, balances = {} }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 anim-fadein" onClick={onClose}>
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-navy-800 card-stroke shadow-card anim-slideup" onClick={e => e.stopPropagation()}>
        <div className="p-5 border-b border-white/5 flex items-center justify-between">
          <div className="text-[15px] font-semibold">Select a token</div>
        ...
        <div className="p-4 border-b border-white/5">
          <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-white/[0.03] input-stroke">
            ...
            <input autoFocus value={q} onChange={e => setQ(e.target.value)} placeholder="Search name or paste address"
                   className="flex-1 bg-transparent outline-none text-[14px] placeholder-white/30"/>
          </div>
          <div className="flex flex-wrap gap-2 mt-3">
            {['USDC','EURC','QETH','MIRA'].map(s => (
              <button key={s} onClick={() => onPick(s)} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full card-stroke hover:bg-white/5 text-[12px]">
        ...
        <div className="max-h-[360px] overflow-y-auto no-scrollbar p-2">
          {filtered.map(t => (
            <button key={t.sym} onClick={() => onPick(t.sym)} className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-white/[0.04] tk text-left">
              <TokenLogo sym={t.sym} size={32}/>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <div className="text-[14px] font-medium">{t.sym}</div>
                  <div className="text-[10px] mono uppercase tracking-wider text-white/40 px-1.5 py-0.5 rounded bg-white/5">{t.tag}</div>
                </div>
