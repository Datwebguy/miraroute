...
function AmountRow({ label, tokenSym, amount, onAmount, onOpenSelect, balance, readOnly, usd, max }) {
  return (
    <div className="rounded-2xl bg-white/[0.025] input-stroke p-4">
      <div className="flex items-center justify-between mb-2">
        <span className="text-[11.5px] mono uppercase tracking-[0.15em] text-white/40">{label}</span>
        {balance != null && (
          <div className="flex items-center gap-2 text-[12px] text-white/55">
            <span>Balance <span className="mono text-white/75">{fmt(balance)}</span></span>
            {max && (
              <>
      ...
      <div className="flex items-center gap-3">
        <input
          ...
          placeholder="0"
          inputMode="decimal"
          className="flex-1 min-w-0 bg-transparent text-[30px] font-light outline-none placeholder-white/20 tracking-tight"
        />
        <button onClick={onOpenSelect}
                className="flex items-center gap-2 pl-1 pr-2 py-1.5 rounded-full bg-white/[0.06] hover:bg-white/[0.09] card-stroke shrink-0">
          <TokenLogo sym={tokenSym} size={24}/>
          <span className="font-medium text-[14px]">{tokenSym}</span>
          <Icons.ChevronDown size={14} className="text-white/60"/>
        </button>
      </div>
      <div className="mt-1.5">
        <span className="text-[12px] mono text-white/40">{usd}</span>
      </div>
    </div>
  );
}
...
function RecipientField({ value, onChange }) {
  ...
  return (
    <div className="rounded-2xl bg-white/[0.025] input-stroke p-4">
      <div className="flex items-center justify-between mb-2">
        <span className="text-[11.5px] mono uppercase tracking-[0.15em] text-white/40">Recipient</span>
        {state === 'ok' && resolved && (
          <span className="text-[11px] mono text-teal-400 flex items-center gap-1">
            <Icons.Check size={11}/> resolved
          </span>
        )}
        {state === 'resolving' && (
          <span className="text-[11px] mono text-white/50 flex items-center gap-1.5">
            <span className="inline-block w-3 h-3 border border-teal-400 border-t-transparent rounded-full spin-slow"/> resolving…
          </span>
        )}
      ...
      {state === 'ok' && resolved && (
        <div className="mt-2 flex items-center gap-2 text-[11.5px] mono text-white/55">
          ...
          <button className="text-white/40 hover:text-teal-400">
            <Icons.Copy size={11}/>
          </button>
        </div>
      )}
...
export function AdvancedSettings({ open, onToggle, slippage, onSlippage, gas, onGas, autoSlip, onAutoSlip }) {
  ...
  return (
    <div className="rounded-2xl card-stroke bg-white/[0.015] overflow-hidden">
      ...
      <div className="expandy" style={{ maxHeight: open ? 260 : 0, opacity: open ? 1 : 0 }}>
        <div className="px-4 pb-4 pt-1 space-y-4 border-t border-white/5">
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="text-[12px] text-white/60 flex items-center gap-1.5">
                Max slippage <Icons.Info size={11} className="text-white/30"/>
              </div>
            ...
            <div className={`flex items-center gap-2 ${autoSlip ? 'opacity-40 pointer-events-none' : ''}`}>
              ...
              <div className="flex-1 flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/[0.03] input-stroke">
                <input value={slippage}
                       onChange={e => onSlippage(parseFloat(e.target.value) || 0)}
          ...
          <div>
            <div className="text-[12px] text-white/60 mb-2 flex items-center gap-1.5">
              Transaction speed <Icons.Info size={11} className="text-white/30"/>
            </div>
    ...
    return (
      <button disabled className="w-full py-4 rounded-2xl font-semibold text-[14px] relative overflow-hidden shimmer text-[#07261F]">
        <span className="relative z-10 flex items-center justify-center gap-2">
          ...
          {state === 'confirming' ? 'Confirm in wallet…' : 'Routing via MiraRoute…'}
        </span>
      </button>
    );
  }
...
export default function SwapCard({
  ...
  fastMode, showRefuel, onDismissRefuel, onRefuel,
  savings, slippage, setSlippage, autoSlip, setAutoSlip,
  gas, setGas, recipient, setRecipient,
  ...
  const usdIn     = fmtUSD(amountNum * fromT.price);
  const usdOut    = fmtUSD(amountOut  * toT.price);
  const insufficient = amountNum > balance;

  const canSwap = isConnected && amountNum > 0 && !insufficient && swapState === 'idle';
  const btnLabel = !isConnected     ? 'Connect wallet'
    ...
    : isLivePair                    ? `Swap ${fromSym} → ${toSym} on Arc`
  ...
  return (
    <div className="w-full max-w-[460px] relative">
      <div className="absolute -inset-6 rounded-[32px] pointer-events-none"
           style={{ background: 'radial-gradient(circle at 50% 0%, rgba(45,212,191,.18), transparent 70%)' }}/>

      <div className="relative rounded-[26px] bg-[#0F1E2E]/80 backdrop-blur card-stroke shadow-card p-5 space-y-3">
        <div className="flex items-center justify-between px-1 pt-0.5 pb-1">
          ...
          <div className="flex items-center gap-1">
            <button title="Refresh quotes" className="p-1.5 rounded-lg hover:bg-white/5 text-white/50 hover:text-white">
              <Icons.Refresh size={14}/>
            </button>
            {!compact && (
              <button onClick={() => setAdvOpen(o => !o)} title="Settings"
                      className={`p-1.5 rounded-lg hover:bg-white/5 ${advOpen ? 'text-teal-400' : 'text-white/50 hover:text-white'}`}>
        ...
        {showRefuel && (
          <GasRefuel visible={showRefuel} onDismiss={onDismissRefuel} onRefuel={onRefuel}/>
        )}
        ...
        <div className="relative h-0">
          <div className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
            <button onClick={flip}
                    className="w-9 h-9 rounded-xl bg-[#0F1E2E] card-stroke flex items-center justify-center hover:bg-[#162638] hover:rotate-180 transition-all duration-300">
        ...
        <AmountRow label="You receive" tokenSym={toSym}
                   ...
                   onOpenSelect={() => onOpenPicker('to')}
                   balance={toBal} usd={usdOut}/>

        <RecipientField value={recipient} onChange={setRecipient}/>

        {!compact && (
          <AdvancedSettings open={advOpen} onToggle={() => setAdvOpen(o => !o)}
        ...
        {!compact && amountNum > 0 && (
          <RoutePreview open={routeOpen} onToggle={() => setRouteOpen(o => !o)}
        ...
        <div className="flex items-center justify-between text-[11px] mono text-white/35 px-1 pt-1">
          <span className="flex items-center gap-1.5">
            <Icons.Lock size={10}/> Non-custodial · MEV shield on
          </span>
